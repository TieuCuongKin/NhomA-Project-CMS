...
function get_comment_author_email($comment_ID = 0)
{
     ...
     * @since 1.5.0
     */
    return apply_filters('get_comment_author_email',
        $comment->comment_author_email, $comment->comment_ID, $comment);
}
...
{ // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
    ...
    return apply_filters('get_comment_author_IP', $comment->comment_author_IP,
        $comment->comment_ID,
        $comment);  // phpcs:ignore WordPress.NamingConventions.ValidHookName.NotLowercase
}
...
function get_comment_author_url($comment_ID = 0)
{
    ...
    if (!empty($comment)) {
        $author_url = ('http://' === $comment->comment_author_url) ? ''
...
function comment_class(
    ...
    $css_class = 'class="' . implode(' ',
            get_comment_class($css_class, $comment, $post_id)) . '"';
...
function get_comment_class($css_class = '', $comment_id = null, $post_id = null)
{
     ...
     *
     */
    return apply_filters('comment_class', $classes, $css_class,
...
function get_comment_excerpt($comment_ID = 0)
{
     ...
     *
     */
    $comment_excerpt_length = apply_filters('comment_excerpt_length',
     ...
     * @since 1.5.0
     */
    return apply_filters('get_comment_excerpt', $excerpt, $comment->comment_ID,
        $comment);
}
...
{ // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
     ...
     *
     */
    return apply_filters('get_comment_ID', $comment_ID,
        $comment);  // phpcs:ignore WordPress.NamingConventions.ValidHookName.NotLowercase
}
...
function get_comment_time($format = '', $gmt = false, $translate = true)
{
     ...
     *
     */
    return apply_filters('get_comment_time', $date, $format, $gmt, $translate,
        $comment);
}
...
function get_trackback_url()
{
    if (get_option('permalink_structure')) {
        $tb_url = trailingslashit(get_permalink())
...
function wp_comment_form_unfiltered_html_nonce()
{
    ...
    if (current_user_can('unfiltered_html')) {
        wp_nonce_field('unfiltered-html-comment_' . $post_id,
            '_wp_unfiltered_html_comment_disabled', false);
...
function comments_template($file = '/comments.php', $separate_comments = false)
{
    ...
    if (get_option('page_comments')) {
        ...
        if ($page) {
             ...
             */
            $top_level_args
                = apply_filters('comments_template_top_level_query_args',
                $top_level_args);
     ...
     *
     */
    $wp_query->comments = apply_filters('comments_array', $comments_flat,
        $post->ID);
    ...
    if ('' == get_query_var('cpage') && $wp_query->max_num_comment_pages > 1) {
        set_query_var('cpage', 'newest' === get_option('default_comments_page')
...
function comments_popup_link(
    ...
    if (false === $zero) {
        ...
        $zero
            = sprintf(__('No Comments<span class="screen-reader-text"> on %s</span>'),
            $post_title);
    }
    ...
    if (false === $one) {
        /* translators: %s: Post title. */
        $one
            = sprintf(__('1 Comment<span class="screen-reader-text"> on %s</span>'),
            $post_title);
    }
    ...
    if (false === $more) {
        ...
        $more
            = _n('%1$s Comment<span class="screen-reader-text"> on %2$s</span>',
    ...
    if (false === $none) {
        /* translators: %s: Post title. */
        $none
            = sprintf(__('Comments Off<span class="screen-reader-text"> on %s</span>'),
            $post_title);
    }

    if (0 == $number && !comments_open() && !pings_open()) {
        echo '<span' . ((!empty($css_class)) ? ' class="' . esc_attr($css_class)
                . '"' : '') . '>' . $none . '</span>';
        return;
    }
...
function get_comment_reply_link($args = array(), $comment = null, $post = null)
{
    ...
    if (get_option('page_comments')) {
        $permalink = str_replace('#comment-' . $comment->comment_ID, '',
    ...
    if (get_option('comment_registration') && !is_user_logged_in()) {
        ...
        $data_attributes = array(
            ...
            'replyto' => sprintf($args['reply_to_text'],
     ...
     *
     */
    return apply_filters('comment_reply_link',
        $args['before'] . $link . $args['after'], $args, $comment, $post);
}
...
function get_cancel_comment_reply_link($text = '')
{
    ...
    $link = esc_html(remove_query_arg(array(
            'replytocom',
            'unapproved',
            'moderation-hash'
        ))) . '#respond';

    $formatted_link = '<a rel="nofollow" id="cancel-comment-reply-link" href="'
        . $link . '"' . $style . '>' . $text . '</a>';
...
function comment_form_title(
    ...
    if (0 == $reply_to_id) {
        ...
        if ($link_to_parent) {
            $author = '<a href="#comment-' . get_comment_ID() . '">'
...
function wp_list_comments($args = array(), $comments = null)
{
    ...
    $defaults = array(
        ...
        'reverse_children' => '',
        'format' => current_theme_supports('html5', 'comment-list') ? 'html5'
            : 'xhtml',
        'short_ping' => false,
        'echo' => true,
    );
    ...
    if (null !== $comments) {
        ...
        if ($parsed_args['page'] || $parsed_args['per_page']) {
            ...
            if (!$current_cpage) {
                $current_cpage = 'newest'
                === get_option('default_comments_page') ? 1
                    : $wp_query->max_num_comment_pages;
            }

            $current_per_page = get_query_var('comments_per_page');
            if ($parsed_args['page'] != $current_cpage
                || $parsed_args['per_page'] != $current_per_page
            ) {
                $comment_args = array(
                    'post_id' => get_the_ID(),
                    'orderby' => 'comment_date_gmt',
    ...
    if ('' === $parsed_args['page']) {
        if (empty($overridden_cpage)) {
            ...
            $parsed_args['page'] = ('newest'
                === get_option('default_comments_page'))
                ? get_comment_pages_count($_comments, $parsed_args['per_page'],
                    $threaded) : 1;
    ...
    if (null === $parsed_args['reverse_top_level']) {
        $parsed_args['reverse_top_level'] = ('desc'
    ...
    $output = $walker->paged_walk($_comments, $parsed_args['max_depth'],
        $parsed_args['page'], $parsed_args['per_page'], $parsed_args);
...
function comment_form($args = array(), $post_id = null)
{
    ...
    /* translators: %s: Asterisk symbol (*). */
        ...
        . __('Required fields are marked %s') . '</span>',
    ...
    $defaults = array(
            ...
            <div class="card" style="border-radius: 5px;">
                <div class="card-header">
                    <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                        <li class="nav-item">
                                <a class="nav-link active" id="posts-tab" data-toggle="tab" role="tab" aria-controls="posts" aria-selected="true">Make
                                    a Post</a>
                        </li>
                    </ul>
                </div>',
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="posts" role="tabpanel" aria-labelledby="posts-tab">
                                <div class="form-group">
                                    ...
                                    <textarea id="comment" name="comment" cols="45" rows="8" maxlength="65525" placeholder="What are you thinking..."'
            . $required_attribute . '></textarea>
                                </div>
                            </div>
                        </div>
                <!--- Post Form Ends -->'
        ...
        'submit_field' => '<div class="text-right">
                        	            <button type="submit" class="btn btn-primary button-blue" style="border-radius: 5px;">%1$s %2$s</button>
                                    </div>
                                </div>
                            </div>',
        'format' => 'xhtml',
    ...
    if (isset($args['fields']['email'])
        && false === strpos($args['comment_notes_before'], 'id="email-notes"')
    ) {
    ...
    <div id="respond" class="<?php echo esc_attr($args['class_container']); ?>">
        ...
        if (get_option('comment_registration') && !is_user_logged_in()) :
            ...
            $comment_fields = array('comment' => $args['comment_field'])
                + (array)$args['fields'];
             ...
             *
             */
            $comment_fields = apply_filters('comment_form_fields',
                $comment_fields);
             ...
             *
             */
            $submit_button = apply_filters('comment_form_submit_button',
                $submit_button, $args);
             ...
             *
             */
            echo apply_filters('comment_form_submit_field', $submit_field,
                $args);
